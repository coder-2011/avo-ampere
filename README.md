...
- Candidate support: Python candidate modules plus CUDA-extension attention candidates, including a BF16 WMMA QK/PV seed accepted through the seq16384 lane.
...
- Research state: the autonomous loop has accepted multiple benchmark lanes, including a seq16384 BF16 WMMA lane. The open result is still scaling toward 32768 and beating FlashAttention-2 on the target suite.
...
- `fix: align mma sequence guard with wrapper` made wrapper-advertised MMA sequence caps match the CUDA guard; follow-up bounded loops advanced the MMA source to accepted seq2048, seq4096, seq8192, and seq16384 lanes.
...
Score the BF16 WMMA QK/PV attention candidate on the accepted seq16384 lane:
...
- Scaling the accepted seq16384 WMMA QK/PV seed toward the remaining long-sequence target shape.
