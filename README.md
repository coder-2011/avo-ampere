...
scores, and `experiments.md` for exact recovery. When the recent tail fits
within `max_chars`, the summary is bounded to the remaining character budget;
...
`evolve-loop` requires `--attempts-dir` so cross-step memory is always available. It stops when a step is accepted, when rejected-patch cleanup fails, when a planner provider/API outage is recorded, when `--max-steps` is exhausted, or between steps when optional `--max-wall-time-s` is reached. Command failures and gate rejections are recorded, summarized into the next prompt, and allowed to continue until one of those stop conditions is reached. Provider outages stop the loop after the recorded step because repeating them cannot improve CUDA search. Attempt summaries also append a supervisor signal when the recent history shows repeated unaccepted command/edit fingerprints, recurring failure classes in the unaccepted tail, or five unaccepted attempts in a row. When such stagnation is detected, summaries include broad strategy-reset candidates from the Ampere search space, while still requiring a scoped `candidate_transform` or a no-edit diagnostic tied to a concrete bottleneck. Recurring promotable classes are written to `preflight_tracks.json` and loaded before materialized transform/patch preflight. Async-copy compile failures are kept as repair feedback (`async_copy_compile_error`) unless they hit a concrete structural invariant, so copy granularity and API iteration do not become hard bans by recurrence alone. Hard worker exits are classified as `worker_crash` for feedback, but are not promoted to hard preflight tracks by recurrence alone.

When an applied edit fails compile, transform materialization, correctness, or crashes an isolated worker, the loop can make a bounded immediate repair request before finalizing the step. The failed edit is reverted first, the repair prompt includes the current compiler/correctness/materialization/crash evidence plus earlier failed repair payloads from that episode, and an unchanged replay of any failed episode payload is rejected before execution. Compile-style repair prompts also extract a compact diagnostic summary with source locations, symbols, and key compiler lines so the next edit can target the actual failure instead of only seeing a class label. Async-copy compile repairs also get class-specific guidance to repair API/include/stage/dataflow issues without treating copy granularity alone as a hard rejection. If the repair decision itself violates the repair contract, the invalid decision is not executed and the agent gets targeted validation feedback before the loop gives up.
