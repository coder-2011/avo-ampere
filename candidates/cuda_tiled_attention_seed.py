...
SOURCE_DIR = Path(__file__).resolve().parent / "cuda_tiled_attention"
...
def _extension():
    ...
    return load(
        name="avo_cuda_tiled_attention_seed",
        sources=[
...
def attention(q, k, v, causal: bool):
    ...
    if seq_len > MAX_SMOKE_SEQUENCE or head_dim > MAX_SMOKE_HEAD_DIM:
        raise RuntimeError(
            "cuda_tiled_attention_seed is a tiny tiled correctness seed; "
